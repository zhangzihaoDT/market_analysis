...
def main(argv: list[str]) -> int:
    ...
    system_prompt = (
        ...
        "5. 🚨【最高优先级红线指令】强制终止原则：\n"
        "   - 只要你通过工具调用（如 SQL 或 Python）获取到了足以回答用户核心问题的数据结果（如整体销量、整体渗透率），**必须在当前轮次立即终止任何进一步的数据探索**。\n"
        "   - **严禁**进行任何画蛇添足的交叉验证、下钻分析（如查各区域分布）、趋势分析（如查历史月份同比/环比），除非用户在问题中明确要求。\n"
        "   - 一旦拿到核心指标，你的下一个回复必须是纯文本的最终答案，**绝对不允许**再输出任何 tool_calls。\n"
    )
